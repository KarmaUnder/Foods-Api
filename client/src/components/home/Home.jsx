           ...
           <div className={styles.title}>
                ...
                <div className={styles.search}>
                    <form onSubmit={handleSubmit}>
                        <input className={styles.searchInput} onChange={handleInputChange} type="text" required placeholder="Food name.." name='name' value={name}/>
                    ...
                    !error ? recipeCurrentPage && recipeCurrentPage.map( el => {
                        return (
                            <Link className={styles.cardText} to={'/recipe/' + el.id} key={el.id}>
                                <Card id={el.id} image={el.image} name={el.name} diets={el.diets} healty={el.healty} />
                            </Link>
                        )
                    }) : <h3>{message}</h3>
                }
